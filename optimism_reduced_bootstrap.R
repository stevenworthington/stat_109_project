...
boot_opt <- function(N, errSD, iter) {
...
parameter_search <- function(grid = pgrid, FUN = boot_opt) {
  future_pmap(
...
###################################################################
# graphs for optimism-reduced bootstrap
...
ggsave(p, file = "Boot_opt_combined.pdf", height = 6, width = 7)   
